import { NsTrinhdoVanhoa } from '../models/ns-trinhdo';
...
export class NsTrinhdoVanhoaService {

  private readonly api = getRoute('trinhdovanhoa/');
  ...
  list(vanhoa_id: number, filter?: { search: string }): Observable<NsTrinhdoVanhoa[]> {
  ...
  delete_vanhoa(id: number): Observable<any> {
    return this.http.delete<Dto>(this.api + id.toString());
  }

  edit_vanhoa(id: number, data: any) {
    return this.http.put<Dto>(this.api + id.toString(), data);
  }


  add_vanhoa(data: any): Observable<any> {
